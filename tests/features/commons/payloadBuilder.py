...
def create_middleware_api_payload_body(json_template_name, edited_json, service, version):
...
def create_middleware_api_payload(json_template_name, data, multiple_request, service, version):
    edited_json = template_editor(json_template_name, data, multiple_request)
    if multiple_request:
        return [create_middleware_api_payload_body(json_template_name, data, service, version) for data
                in edited_json]
    return create_middleware_api_payload_body(json_template_name, edited_json, service, version)
...
def create_payload(template_name, data, service, method, version, multiple_request=False, request_through_middleware_api=False):
    if request_through_middleware_api:
        payload = create_middleware_api_payload(template_name, data, multiple_request, service, version)
